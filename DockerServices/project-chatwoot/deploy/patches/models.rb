module Llm::Models
  ...
  OPENAI_COMPATIBLE_PROVIDERS = %w[openai custom_openai].freeze
  ...
  class << self
    ...
    def openai_override_options(model_name, declared_provider: nil)
      provider = declared_provider || provider_for(model_name)
      return {} unless OPENAI_COMPATIBLE_PROVIDERS.include?(provider)

      registry_model = begin
        RubyLLM.models.find(model_name)
      rescue RubyLLM::ModelNotFoundError
        nil
      end
      return {} if registry_model&.provider == 'openai'
