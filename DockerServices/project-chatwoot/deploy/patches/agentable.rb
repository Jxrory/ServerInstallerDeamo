module Concerns::Agentable
  ...
  def agent
    Agents::Agent.new(
      ...
      tools: agent_tools,
      model: agent_model,
      **Llm::Models.openai_override_options(agent_model),
      temperature: temperature.presence&.to_f || DEFAULT_TEMPERATURE,
      response_schema: supports_structured_output? ? agent_response_schema : nil
    )
  end

  def supports_structured_output?
    Llm::Models.provider_for(agent_model) != 'custom_openai'
  end
