...
class Llm::BaseAiService
  ...
  def chat(model: @model, temperature: @temperature)
    RubyLLM.chat(model: model, **llm_provider_options).with_temperature(temperature)
  end
  ...
  def setup_model
    route = feature_route
    if account_override_route?(route) || captain_v2_assistant?
      @model = route[:model]
    ...
    @llm_provider = Llm::Models.provider_for(@model) || route&.dig(:provider)
  end

  def llm_provider_options
    Llm::Models.openai_override_options(@model, declared_provider: @llm_provider)
  end
